...
class ElfFlameGenerator:
    ...
    def generate(self, elf, static_libraries, output):
        ...
        with subprocess.Popen([self._readelf, "--symbols", "--wide", "--sym-base=10", elf], stdout=subprocess.PIPE) as p:
            for line in p.stdout:
                match = re.fullmatch(r"\s*(\d+):\s+(?P<addr>\w+)\s+(?P<size>\d+)\s+(?P<type>\w+)\s+(?P<bind>\w+)\s+(?P<vis>\w+)\s+(?P<ndx>\w+)\s+(?P<name>.+)", line.decode('utf-8').strip())
                if not match:
                    continue
        ...
        with subprocess.Popen([self._nm, "--line-numbers", "--print-file-name", elf, *static_libraries], stdout=subprocess.PIPE) as p:
            for line in p.stdout:
                match = re.fullmatch(r"(?P<infile>[^:]+):(?:(?P<object>[^:]+):)?(?P<addr>\w+)\s+(?P<section>\w+)\s+(?P<name>\S+)(?:\s+(?P<file>.+?):(?P<line>\d+))?", line.decode('utf-8').strip())
                if not match:
                    continue
...
if __name__ == "__main__":
    ...
    parser.add_argument('-a', '--library', metavar='STATIC_LIBRARY', dest='libraries', action='append', default=[], help='Specify additional static libraries to find symbols in')
